...
def test_constructor_rejects_nan_theta_kl() -> None:
    with pytest.raises(ValueError, match="theta_kl must be finite and > 0"):
        AdaptiveLagInferer(theta_kl=float("nan"))
...
def test_constructor_rejects_bool_and_non_integral_m_consec() -> None:
    # bool is an int subclass in Python; m_consec=True must not silently become m_consec=1.
    with pytest.raises(ValueError, match="must be an int, not bool"):
        AdaptiveLagInferer(m_consec=True)
    # m_consec=2.5 must not silently truncate to 2 via int().
    with pytest.raises(ValueError, match="must be an integer value"):
        AdaptiveLagInferer(m_consec=2.5)
...
def test_get_metadata_matches_last_infer_result() -> None:
    P = _constant_trajectory(n_lags=8, T=10, row=[0.9, 0.05, 0.05])
    inferer = AdaptiveLagInferer()
    result = inferer.infer(P)
    assert inferer.get_metadata() == result.metadata
...
def test_transform_returns_as_of_posterior() -> None:
    """transform() is the PosteriorTransformer entry point: it must equal infer().as_of_posterior()
    exactly, not just agree on argmax — pipeline consumers rely on the full (T, K) matrix."""
    P = _constant_trajectory(n_lags=8, T=15, row=[0.7, 0.2, 0.1])
    inferer = AdaptiveLagInferer()

    transformed = inferer.transform(P)
    expected = inferer.infer(P).as_of_posterior()

    np.testing.assert_allclose(transformed, expected)


def test_transform_conforms_to_posterior_inferer_protocol() -> None:
    """PosteriorTransformer is not @runtime_checkable, so conformance is duck-typed: transform()
    must exist, be callable, and accept a single positional array argument."""
    inferer = AdaptiveLagInferer()
    assert hasattr(inferer, "transform")
    assert callable(inferer.transform)


def test_transform_output_shape_and_simplex_rows() -> None:
    P = _constant_trajectory(n_lags=8, T=15, row=[0.7, 0.2, 0.1])
    out = AdaptiveLagInferer().transform(P)
    assert out.shape == (15, 3)
    np.testing.assert_allclose(out.sum(axis=1), 1.0, atol=1e-12)
...
def test_compute_trajectories_rejects_bool_and_non_integral_kmax() -> None:
    class _FakeHmm:
        def predict_proba_fixed_lag_sweep(self, X, lags, window_size=None):
            return {}

    # k_max=2.5 must fail at the API boundary rather than deep inside range().
    with pytest.raises(ValueError, match="must be an integer value"):
        compute_trajectories(_FakeHmm(), np.zeros((10, 2)), k_max=2.5)
    with pytest.raises(ValueError, match="must be an int, not bool"):
        compute_trajectories(_FakeHmm(), np.zeros((10, 2)), k_max=True)
