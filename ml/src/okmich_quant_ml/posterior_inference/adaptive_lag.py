...
inferer commits at the earliest lag where a stability criterion fires; bars that never satisfy the
criterion fall back to ``k_max`` (oracle-equivalent under the fixed-lag smoother).
...
class AdaptiveLagInferer:
    ...
    def __init__(self, theta_kl: float = 0.005,
        ...
        self.m_consec = int(m_consec)
        if self.criterion != StabilityCriterion.MAP_PERSISTENCE and self.theta_kl <= 0.0:
            raise ValueError(
                f"theta_kl must be > 0 for criterion={self.criterion.value}, got {self.theta_kl}"
            )
        if self.m_consec < 1:
...
def compute_trajectories(hmm: Any, X: NDArray, k_max: int, window_size: int | None = 48) -> NDArray:
    """Compute lag trajectories for ``AdaptiveLagInferer.infer()``.
