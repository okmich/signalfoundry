...
# copied from https://github.com/YuxuanXiu/Financial-Time-Series-Trend-Labeling/tree/main
def _auto_labeling(data_list, timestamp_list, w):
...
def auto_label(series: pd.Series, omega: float):
    data_list = series.to_list()
    timestamp_list = series.index.to_list()
    # call the delegate function
    labelled_series, _ = _auto_labeling(data_list, timestamp_list, omega)
    tuner = RemainingValueTuner()
    tuned_labels = np.array(
        tuner.tune(
            data_list,
            list(labelled_series),
            enforce_monotonicity=True,
            normalize_over_interval=True,
        )
    )
    return pd.Series(labelled_series, index=series.index), pd.Series(np.array(tuned_labels), index=series.index)
