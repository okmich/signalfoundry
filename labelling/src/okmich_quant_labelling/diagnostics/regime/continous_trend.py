...
def bi_ctl_label(series: pd.Series, omega: float, smoothing_window: int = 36) -> tuple[pd.Series, pd.Series]:
...
def tri_ctl_label(series: pd.Series, marginal_change_thres: float, window_size: int,
                  smoothing_window: int = 36) -> tuple[Series, Series]:
    prices = series.to_list()

    labeller = TernaryCTL(marginal_change_thres=marginal_change_thres, window_size=window_size)
    labelled_prices = labeller.get_labels(prices)

    tuner = RemainingValueTuner()
    linear_smoother = LinearWeightedAverage(window_size=smoothing_window, direction="centered")
    tuned_labels_with_linear_smoothing = np.array(
        tuner.tune(
            prices,
            labelled_prices,
...
def optimize_bi_ctl_label(series: pd.Series, omega_bounds: tuple[float, float] = (0, 0.1),
                          fees: float = 0.0) -> Dict[str, Any]:
    fees_config = FeesConfig(lp_transaction_fees=fees, sp_transaction_fees=fees)
    custom_bounds = {"omega": omega_bounds,}
    optimizer = Optimizer(ReturnsEstimatorWithFees(fees_config), initial_points=5, nb_iter=100)
    optimization_results = optimizer.optimize(BinaryCTL, series.to_list(), bounds=custom_bounds, verbose=1)
    return optimization_results["params"]


def optimize_tri_ctl_label(series: pd.Series, marginal_change_thres_values: tuple[float, float],
                           window_size_values: tuple[int, int], holding_fees: float = 0.0,
                           txn_fees: float = 0.0) -> Dict[str, Any]:
    fees_config = FeesConfig(
        lp_transaction_fees=txn_fees, sp_transaction_fees=txn_fees,
        lp_holding_fees=holding_fees, sp_holding_fees=holding_fees)
    custom_bounds = {
    ...
    optimization_results = optimizer.optimize(TernaryCTL, series.to_list(), custom_bounds, verbose=1)
    return optimization_results["params"]
