...
_VOLUME = [
    ...
    _fe("chaikin_oscillator", "volume", "order_flow",
        "Chaikin Oscillator — EMA_fast(AD) − EMA_slow(AD), MACD on the A/D line",
        rr=M, ret=M, dr=H, hor=S, wbi=[TR], dir_=True, vol=True,
        notes="Smoother than CMF/MFI on intraday bars; 12/26 EMA defaults (textbook 3/10 for daily)"),
...
_MOMENTUM = [
    ...
    _fe("tick_volume_indicator", "momentum", "momentum",
        ...
        notes="Volume-based momentum; |ret| IC sign is asset-dependent (positive on FX/crypto, "
              "negative on gold) — validate per-asset. Near-zero correlation with delta_obv (inter-bar)."),
    # Proximity & range
