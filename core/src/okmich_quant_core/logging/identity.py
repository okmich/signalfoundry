...
def _resolve_log_base(log_base: str | Path | None = None) -> Path:
    """Resolve this runner's log directory (LOGGING_CONTRACT §10): the log root, plus the account folder the
    running system is deployed in, so the log tree mirrors the live tree — ``<log_root>\\<account>`` for a system
    in ``<live_base>\\<account>\\``, the bare ``<log_root>`` for one deployed anywhere else (dev, flat).

    ``log_root`` is the explicit ``log_base`` or ``OKMICH_QUANT_LOG_BASE``. There is deliberately no hardcoded
    production fallback: deployment paths such as ``D:\\quant_logs`` are ops examples, not portable source-code
    defaults. Shared by the bar logger, the runner status writer and the text log so all resolve the same directory.
    """
    root = _resolve_log_root(log_base)
...
def runner_log_dir(runner_root: str, log_base: str | Path | None = None) -> Path:
    """A runner's own log folder, beside its ``status.json``: ``<log_base>/[<account>/]<runner_root>``. For files a
    runner or strategy keeps outside the contract channels (text log, persisted state), so nothing is written at
    the log root."""
