#
# ADAPTED FROM https://raw.githubusercontent.com/philipperemy/keras-tcn/refs/heads/master/tcn/tcn.py
# Had to isolate because of dependency issues
...
class ResidualBlock(Layer):
    ...
    def __init__(self, dilation_rate: int, nb_filters: int, kernel_size: int, padding: str, activation: str = "relu",
                 dropout_rate: float = 0, kernel_initializer: str = "he_normal", use_batch_norm: bool = False,
        ...
        with K.name_scope(self.name):  # name scope used to make sure weights get unique names
            ...
            for k in range(2):  # dilated conv block.
                ...
                with K.name_scope("act_and_dropout_{}".format(k)):
                    self._build_layer(
                        Activation(self.activation, name="Act_Conv1D_{}".format(k))
                    )
                    self._build_layer(
                        SpatialDropout1D(rate=self.dropout_rate, name="SDropout_{}".format(k))
                    )
...
class TCN(Layer):
    ...
    def __init__(self, nb_filters=64, kernel_size=3, nb_stacks=1, dilations=(1, 2, 4, 8, 16, 32), padding="causal",
                 use_skip_connections=True, dropout_rate=0.0, return_sequences=False, activation="relu",
                 kernel_initializer="he_normal", use_batch_norm=False, use_layer_norm=False, go_backwards=False,
                 return_state=False, **kwargs):
        # Validate before touching Keras internals
        if use_batch_norm + use_layer_norm > 1:
            raise ValueError("Only one normalization can be specified at once.")

        if isinstance(nb_filters, list):
            assert len(nb_filters) == len(dilations)
            if len(set(nb_filters)) > 1 and use_skip_connections:
                raise ValueError(
        ...
        # Initialize parent class before setting own attributes
        ...
        if self.padding == "same":
            ...
            if (
                ...
                self.output_slice_index = int(
                    self.tolist(self.build_output_shape)[1] / 2
                )
            else:
...
def compiled_tcn(
    ...
    x = TCN(nb_filters, kernel_size, nb_stacks, dilations,
            padding, use_skip_connections, dropout_rate,
            return_sequences, activation, kernel_initializer,
            use_batch_norm, use_layer_norm, name=name)(input_layer)
